...
import type { CloudProjectionCollisionQuarantine } from "./cloud-provider-adapter";
import type { CloudAccountProjectionStore, CloudLinkCollisionStore } from "./linking";
export class PrismaCloudProjectionStore implements CloudAccountProjectionStore {
  async link(context: ProviderOperationContext, input: Parameters<CloudAccountProjectionStore["link"]>[1]) {
    return withTenantDb({ organizationId: context.organizationId, tenantId: context.tenantId }, async (tx) => {
      const [connection, subject] = await Promise.all([
        tx.providerConnection.findFirst({ where: { id: context.providerConnectionId, organizationId: context.organizationId, providerType: input.providerType } }),
        tx.subject.findFirst({ where: { id: input.subjectId, organizationId: context.organizationId, tenantId: context.tenantId } }),
      ]);
      if (!connection || !subject) throw new ProviderAdapterError({ code: "INVALID_SCOPE", message: "Cloud projection target is outside tenant scope" });
      ...
      if (existing) { if (existing.tenantId !== context.tenantId || existing.subjectId !== input.subjectId)
        throw new ProviderAdapterError({ code: "CONFLICT", message: "Cloud projection is already linked", safeDetails: { reason: "EXTERNAL_ID_COLLISION" } });
      ...
      try { const created = await tx.identityAccount.create({ data: { organizationId: context.organizationId, tenantId: context.tenantId,
        ...
        throw new ProviderAdapterError({ code: "CONFLICT", message: "Concurrent cloud projection collision", safeDetails: { reason: "EXTERNAL_ID_COLLISION" } }); throw error; }
    });
  }
}
export class PrismaCloudCollisionStore implements CloudLinkCollisionStore, CloudProjectionCollisionQuarantine {
  async quarantine(context: ProviderOperationContext, input: Parameters<CloudLinkCollisionStore["quarantine"]>[1] |
    Parameters<CloudProjectionCollisionQuarantine["quarantine"]>[1]): Promise<void> {
